...
/**
 * 为了将DLX交换器类型改成topic而对RabbitExchangeQueueProvisioner进行的改造
 * 此代码内容和RabbitExchangeQueueProvisioner 完全一样，除了 276 将DirectExchange改成TopicExchange
 * @author zenglw
 * @date 2019/4/22
 */
public class RabbitExchangeQueueTopicDlxProvisioner extends RabbitExchangeQueueProvisioner {
    ...
    public RabbitExchangeQueueTopicDlxProvisioner(ConnectionFactory connectionFactory) {
    ...
    private void autoBindDLQ(final String baseQueueName, String routingKey, RabbitCommonProperties properties) {
        ...
        if (autoBindDlq) {
            ...
            // 改成topic
            final TopicExchange dlx = new TopicExchange(dlxName);
            declareExchange(dlxName, dlx);
            BindingBuilder.TopicExchangeRoutingKeyConfigurer bindingBuilder = BindingBuilder.bind(dlq).to(dlx);
            Binding dlqBinding;
